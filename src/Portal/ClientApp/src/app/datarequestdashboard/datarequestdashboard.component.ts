...
import { SetStatusResult, DefaultClientEmailGridSettings, ClientEmail, GetClientEmailDataResult, AffiliateInfo, ReturnAffiliates, GridSettings, GetDataRequestDataResult, FavoriteClickedResult } from '../models/misc-models';
...
@Component({
  selector: 'app-datarequestdashboard',
  templateUrl: './datarequestdashboard.component.html',
  styleUrls: ['./datarequestdashboard.component.scss']
})
export class DataRequestDashboardComponent implements OnInit {
  ...
  public getGridData() {
    ...
    const url: string = this.baseUrl + 'api/Portal/GetDataRequestData'
    this.httpClient.get<GetDataRequestDataResult>(url, { headers: this.authService.getRequestHeaders() }).subscribe(result => {
      if (result) {
