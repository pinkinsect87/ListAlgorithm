...
CREATE PROCEDURE [sp_GetTrustIndexScore]
	@Filter VARCHAR(1000) = '',
	@ClientId VARCHAR(10), 
	@EngagementId VARCHAR(10), 
	@StatementIdsFilter VARCHAR(1000) = ''
AS
BEGIN
	...
	IF @StatementIdsFilter <> '' BEGIN
		SET @dsql += '
		AND LS.[StatementId] in (' + @StatementIdsFilter + ')'
	END
	...
	C45 AS (
		SELECT COUNT(1) as count_45 ' +
		@dsql + '
		AND LS.[Response] IN (4, 5) 
	)

	SELECT CAST(CASE WHEN T.[count_total] > 0 THEN C45.[count_45] * 1.0 / T.[count_total] ELSE 0 END AS decimal(18,2)) AS TrustIndexScore
	FROM T, C45;'
